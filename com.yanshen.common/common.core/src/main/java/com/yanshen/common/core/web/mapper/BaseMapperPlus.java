package com.yanshen.common.core.web.mapper;
...
public interface BaseMapperPlus<M, T, V> extends BaseMapper<T> {
    ...
    /**
     * @Description: 针对自定义sql再mapper.xml中执行后进行结果分页自定义方法-20231020版本当前只支持直接使用vo版本
     * @Param: [page, list]
     * @return: P
     * @Author: baohao-jia
     * @Date: 10:01 2023-10-20
     */
    default <T, P extends IPage<T>> P selectVoPageCustomSql(IPage<T> page, List<T> list) {
        //判定传入的集合是否为空，为空则直接响应失败
        if (CollUtil.isEmpty(list)) {
            return (P) new Page<>(page.getCurrent(), page.getSize(), 0L);
        } else {
            IPage<T> voPage = new Page<>(page.getCurrent(), page.getSize(), new PageInfo(list).getTotal());
